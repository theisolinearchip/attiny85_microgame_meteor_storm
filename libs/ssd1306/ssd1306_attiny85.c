...
void ssd1306_send_multiple_equal_data(int length, char data) {
	i2c_start();
	i2c_write_byte(SSD1306_I2C_WRITE_ADDRESS);
	i2c_write_byte(SSD1306_CONTROL_BYTE_ONE_DATA); // no need to repeat different SSD1306_CONTROL_BYTE_MULTIPLE_DATA
	for (int i = 0; i < length; i++) {
		i2c_write_byte(data);
	}
	i2c_stop();
